...
function VerifyPhoneNumberOTP({route, navigation}) {
    ...
    return (
      <Pressable style={{ flex: 1, alignItems: 'center', backgroundColor:'black'}} onPress={Keyboard.dismiss}>
         ...
         <View style={styles(this.props).container}>
          ...
          {isLoading ? <ActivityIndicator size="small" color="white" /> : <Text style={{color:'#81cd2c', top: 28 +'%'}} onPress={() => handleResendCode(phoneNumber)}>Envoyer un autre code</Text>}
            <TouchableHighlight disabled={!isPinReady} style={styles(this.props).groupBtn} onPress={() => handleSubmit(otpCode, phoneNumber)}>
                <View style={styles(this.props).Btn}>
                    <View style={styles(isPinReady).content}>
                        {isLoading ? <ActivityIndicator size="small" color="white" /> : <Text  style={styles(this.props).IText}>Vérifier</Text>}
                    </View>
                </View>
            </TouchableHighlight>
